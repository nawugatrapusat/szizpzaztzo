                ...
                <td style="padding-bottom: 15px;">
                    <table>
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td><span style="color:red;">*</span> <input  autocomplete="off" type="text" name="tanggalFaktur" id="date" size="10" value="<?php echo $penjualanById == '' ? '' : $penjualanById->d . '-' . $penjualanById->m . '-' . $penjualanById->y ?>"/></td>
                        </tr>
                        <?php
                        if ($typeForm != 0) {
//                            echo '
...
//                        </tr>';
            ...
            <table class="table1" border="1">
                ...
                for ($f = 1; $f <= 50; $f++) {
                    ...
                    ?>
                        ...
                        if ($product != '') {
                            foreach ($product as $hasil1) {
                                ...
                                echo '<option ' . $a . ' stock="'.$hasil1->stock.'" value="' . $hasil1->id . '">' . ucwords($hasil1->nama) . ' - ' . ucwords($hasil1->berat) . ' gr | Stock : '.$hasil1->stock.'</option>';
                            }
                        }
                        ?>
                    </select>
                    ...
                    <td>
                        <input class="cetakHargaJual" type="text" value="<?php echo $paramHargaJualDiskon[$f] == '' ? '' : 'Rp. ' . number_format($paramHargaJualDiskon[$f], 0, ',', '.') ?>" disabled/>
                    </td>
                    <td>
                        ...
                        <input type="hidden" class="tmpJumlah<?php echo $f; ?>" name="tmpJumlah<?php echo $f; ?>" value="<?php echo $paramTmpJumlah[$f] == '' ? '0' : $paramTmpJumlah[$f] ?>"/>
                        ...
                        <input type="hidden" class="hargaJualDiskon" name="hargaJualDiskon<?php echo $f; ?>" value="<?php echo $paramHargaJualDiskon[$f] == '' ? '' : $paramHargaJualDiskon[$f] ?>"/>
                        <input type="hidden" class="perhitungan" name="perhitungan<?php echo $f; ?>" value="<?php echo $paramPerhitungan[$f] == '' ? '' : $paramPerhitungan[$f] ?>"/>
    ...
    <script>
        function validateForm() {
            ...
            if ($('#idClient').val() == "") {
                alert("Client Masih Kosong !!!");
                return false;
            }
            if ($('#idEmployeePic').val() == "") {
                alert("Pembawa Masih Kosong !!!");
                return false;
            }
            if ($('#diskon').val() != "tidak" && $("#jumlahDiskon").val() == '') {
                alert("Nominal / Persen Diskon Masih Kosong !!!");
                return false;
            }
            for (a = 1; a <= 50; a++) {
                ...
                if ($('.cekVal1' + a).val() != '' && $('.cekVal2' + a).val() != '') {
                    if (isNaN($('.cekVal2' + a).val()) == true) {
                        alert("Jumlah Ada Yang Tidak Menggunakan Angka !!!");
                        return false;
                    } else if ($('.cekVal2' + a).val() % 1 != 0) {
                        alert("Jumlah Ada Yang Desimal !!!");
                        return false;
                    }
                    if((parseInt($('.cekVal1' + a+ ' option:selected').attr('stock')) + parseInt($('.tmpJumlah' + a).val())) < parseInt($('.cekVal2' + a).val())){
                        alert($('.cekVal1' + a+ ' option:selected').text()+", Tidak Memenuhi Stock !!!");
                        return false;
                    }
        ...
        $(document).ready(function () {
            ...
            $(".idProduct").change(function () {
                ...
                if ($("#idClient").val() != '') {
                    ...
                    product.parent().parent().find('.hargaJualDiskon').val('');
                    product.parent().parent().find('.perhitungan').val('');
                    ...
                    if (product.val() != '') {
                        $.ajax({
                            ...
                            product.parent().parent().find('.hargaJualDiskon').val(data.hargaJualDiskon);
                            product.parent().parent().find('.perhitungan').val(data.perhitungan);
